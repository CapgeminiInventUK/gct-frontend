...
const StatsBanner = (): JSX.Element => {
  ...
  const completed: GradedCompetencies = competencies.reduce(
    (acc, curr) => {
      if (
        curr.checked &&
  ...
  return (
    <div className={styles.container}>
      {(Object.keys(completed) as (keyof GradedCompetencies)[]).map(
        (key: keyof GradedCompetencies) => {
          ...
          return (
            <CustomCircularProgress
              ...
              label={key}
              total={gradeTotal}
              completed={completed[key].length}
            />
          );
        }
      )}
